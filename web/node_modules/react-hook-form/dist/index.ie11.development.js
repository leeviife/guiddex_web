...
function attachEventListeners(_a) {
    var ref = _a.field.ref, handleChange = _a.handleChange, isRadioOrCheckbox = _a.isRadioOrCheckbox;
...
var getFieldsValues = (function (fieldsRef, unmountFieldsStateRef, search) {
    ...
    var _loop_1 = function (name_1) {
        if (isUndefined(search) ||
            ...
            output[name_1] = getFieldValue(fieldsRef, name_1, unmountFieldsStateRef);
        }
    };
...
var compareObject = (function (objectA, objectB) {
    ...
    return (objectAKeys.length === Object.keys(objectB).length &&
        objectAKeys.every(function (key) { return objectB[key] && objectB[key] === objectA[key]; }));
});
            ...
            switch (_p.label) {
                case 0:
                    ...
                    if (required &&
                        ...
                        if (requiredValue) {
                            error[name] = __assign({ type: INPUT_VALIDATION_RULES.required, message: requiredMessage, ref: isRadioOrCheckbox ? (_o = fields[name].options) === null || _o === void 0 ? void 0 : _o[0].ref : ref }, appendErrorsCurry(INPUT_VALIDATION_RULES.required, requiredMessage));
                            if (!validateAllFieldCriteria) {
...
var getPath$1 = (function (parentPath, value) { return getPath(parentPath, value).flat(Infinity); });
...
var skipValidation = (function (_a) {
    var isOnChange = _a.isOnChange, hasError = _a.hasError, isBlurEvent = _a.isBlurEvent, isOnSubmit = _a.isOnSubmit, isReValidateOnSubmit = _a.isReValidateOnSubmit, isOnBlur = _a.isOnBlur, isReValidateOnBlur = _a.isReValidateOnBlur, isSubmitted = _a.isSubmitted;
...
function useForm(_a) {
    ...
    var validateAllFieldCriteria = criteriaMode === VALIDATION_MODE.all;
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, validateField(fieldsRef, validateAllFieldCriteria, fieldsRef.current[name], unmountFieldsStateRef)];
                case 1:
                    error = _a.sent();
    ...
    var executeSchemaOrResolverValidation = React.useCallback(function (payload) { return __awaiter(_this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0: return [4 /*yield*/, resolverRef.current(getValues(), contextRef.current, validateAllFieldCriteria)];
                case 1:
    ...
    var setInternalValues = React.useCallback(function (name, value, config, parentFieldName) {
        for (var key in value) {
            var fieldName = "" + (parentFieldName || name) + (isArray(value) ? "[" + key + "]" : "." + key);
                ...
                return __generator(this, function (_c) {
                    switch (_c.label) {
                        case 0:
                            ...
                            return [4 /*yield*/, resolver(getValues(), contextRef.current, validateAllFieldCriteria)];
                        case 1:
                            ...
                            return [3 /*break*/, 4];
                        case 2: return [4 /*yield*/, validateField(fieldsRef, validateAllFieldCriteria, field, unmountFieldsStateRef)];
                        case 3:
                            error = _c.sent();
                            _c.label = 4;
                        case 4:
    ...
    var getValue = function (name) { return getFieldValue(fieldsRef, name, unmountFieldsStateRef); };
    function getValues(payload) {
        ...
        if (isArray(payload)) {
            return payload.reduce(function (previous, name) {
                var _a;
                return (__assign(__assign({}, previous), (_a = {}, _a[name] = getValue(name), _a)));
            }, {});
        }
        return transformToNestObject(getFieldsValues(fieldsRef, unmountFieldsStateRef));
    }
    var validateResolver = React.useCallback(function (values) {
        if (values === void 0) { values = {}; }
        resolverRef.current(__assign(__assign(__assign({}, defaultValuesRef.current), getValues()), values), contextRef.current, validateAllFieldCriteria).then(function (_a) {
    ...
    var removeFieldEventListener = React.useCallback(function (field, forceDelete) {
        findRemovedFieldAndRemoveListener(fieldsRef, handleChangeRef.current, field, unmountFieldsStateRef, shouldUnregister, forceDelete);
    }, [shouldUnregister]);
        ...
        if (field &&
            ...
            if (shouldUnregister) {
                ...
                [
                    ...
                    validFieldsRef,
                    watchFieldsRef,
                ].forEach(function (data) { return data.current.delete(field.ref.name); });
    ...
    function registerFieldsRef(ref, validateOptions) {
        ...
        if (resolver && !isFieldArray && readFormStateRef.current.isValid) {
            ...
            if (!isOnSubmit && readFormStateRef.current.isValid) {
                validateField(fieldsRef, validateAllFieldCriteria, field, unmountFieldsStateRef).then(function (error) {
                    var previousFormIsValid = isValidRef.current;
                    isEmptyObject(error)
                        ? validFieldsRef.current.add(name)
                        : (isValidRef.current = false);
        ...
        return __generator(this, function (_e) {
            switch (_e.label) {
                case 0:
                    ...
                    fieldErrors = {};
                    fieldValues = __assign(__assign({}, unmountFieldsStateRef.current), getValues());
                ...
                case 1:
                    ...
                    return [4 /*yield*/, resolverRef.current(fieldValues, contextRef.current, validateAllFieldCriteria)];
                case 2:
                ...
                case 4:
                    ...
                    return [4 /*yield*/, validateField(fieldsRef, validateAllFieldCriteria, field, unmountFieldsStateRef)];
                case 5:
                    fieldError = _e.sent();
...
function prepend(data, value) {
    return __spread((isArray(value) ? value : [value || null]), data);
}

function insert(data, index, value) {
    return __spread(data.slice(0, index), (isArray(value) ? value : [value || null]), data.slice(index));
}
...
var Controller = function (_a) {
    ...
    var methods = useFormContext();
    var _c = control || methods.control, defaultValuesRef = _c.defaultValuesRef, setValue = _c.setValue, register = _c.register, unregister = _c.unregister, errorsRef = _c.errorsRef, trigger = _c.trigger, _d = _c.mode, isOnSubmit = _d.isOnSubmit, isOnBlur = _d.isOnBlur, isOnChange = _d.isOnChange, _e = _c.reValidateMode, isReValidateOnBlur = _e.isReValidateOnBlur, isReValidateOnSubmit = _e.isReValidateOnSubmit, isSubmittedRef = _c.isSubmittedRef, touchedFieldsRef = _c.touchedFieldsRef, readFormStateRef = _c.readFormStateRef, reRender = _c.reRender, fieldsRef = _c.fieldsRef, fieldArrayNamesRef = _c.fieldArrayNamesRef, unmountFieldsStateRef = _c.unmountFieldsStateRef, formState = _c.formState;
    ...
    var _f = __read(React.useState(getInitialValue()), 2), value = _f[0], setInputStateValue = _f[1];
    ...
    var commonTask = function (event) {
        var data = getInputValue(event[0], isCheckboxInput);
    ...
    var props = __assign(__assign(__assign({}, rest), { onChange: onChange,
        onBlur: onBlur }), (_b = {}, _b[isCheckboxInput ? 'checked' : VALUE] = value, _b));
    return as
